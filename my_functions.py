...
def save_data_to_csv(edf_path=None, singel_edf_path=None, epoch_len=10, reverse_sort=False, test_run=False, include_score=True):
    filelist = [f for f in os.listdir(edf_path) if f.endswith('.edf')]
    filelist.sort()
    if reverse_sort:
    ...
    for file in filelist:
        ...
        print(file)
        file_firstname = file.split('.edf')[0]
        if os.path.exists(f"{edf_path}{file_firstname}.csv"):
            print("################################################")
            print(f"{edf_path}{file_firstname}.csv exists")
            print("################################################")
            continue

        edf_filepath = edf_path + file
...
#        if test_run:
#            print("test run")
#            raw = raw.crop(tmax=100, include_tmax=False)

        raw.filter(1., 40., fir_design='firwin')
        raw_data = raw.get_data()
        
        # clip eeg data to 200pA and emg data to 100pA
        print("clip eeg data to 200pA and emg data to 100pA")
        raw_data[0][raw_data[0]>0.0002] = 0.0002
        raw_data[1][raw_data[1]>0.0002] = 0.0002
        raw_data[2][raw_data[2]>0.0001] = 0.0001
        raw_data[0][raw_data[0]<-0.0002] = -0.0002
        raw_data[1][raw_data[1]<-0.0002] = -0.0002
        raw_data[2][raw_data[2]<-0.0001] = -0.0001
        ...
        df['epoch_id'] = file[:-4] + '___' + df.index.astype('str')
        df['subject_id'] = file[:-4]
        df['score'] = score_list
        df.to_csv(edf_path + file_firstname + '.csv')
...
def save_single_edf_to_csv_2eeg(edf_filepath=None, epoch_len=10, model_name=None, test_run=False, include_score=False):
    ...
    if os.path.exists(f"{edf_folderpath}{file_firstname}_features.csv"):
        print("##############################################")
        print(f"{edf_folderpath}{file_firstname}_features.csv exists; skipped extracting features for this file.")
        print("##############################################")
        return f"{edf_folderpath}{file_firstname}_features.csv exists; skipped extracting features for this file."
    ...
    downsampled_files = f"{edf_folderpath}{file_firstname}_rs_100hz.npy"
    if not os.path.exists(downsampled_files):
        ...
        with open(f"{edf_folderpath}{file_firstname}_rs_100hz.npy", 'wb') as f:
